...
export const chatRouter = createTRPCRouter({
  promptWithPDFChunks: publicProcedure
    ...
    .mutation(async ({ input: { data } }) => {
      ...
      try{
        if (data.length === 1) {
          const relivantPDFChunks =
            await retriever.getRelivantChunks(mostRecentPrompt);
          geminiResponse = await gemini.promptWithChunks(
            data,
            relivantPDFChunks.map(
              (chunk) => `title: ${chunk.iqpTitle}: content: ${chunk.text}`,
            ),
          );
        } else {
  ...
  db: publicProcedure.input(promptWithDBSchema).mutation(async ({ input }) => {
    const gemini = new Gemini();
